...
def get_group_trans_mats(labels, label_group, group, syll_include, normalize="bigram"):
    ...
    for plt_group in group:
        ...
        freq = get_frequencies(use_labels)
        if len(freq) < max_len:
            freq = np.concatenate((freq, [0.0]*(max_len-len(freq))))
            # freq += [0.0]*(max_len-len(freq))
        group_frequencies = freq[syll_include]
...
def visualize_transition_bigram(
    ...
    for i, g in enumerate(group):
        ...
        axs[i].set_title(g)
        axs[i].set_xticks(np.arange(len(syll_include)), syll_names)
