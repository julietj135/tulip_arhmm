...
from jax_moseq.models.keypoint_slds.alignment import vector_to_angle
...
def load_config(config_path, check_if_valid=True, build_indexes=True):
...
def update_config(config_path, **kwargs):
    ...
    To update `trans_hypparams['kappa']` to `100`::
      ...
      100
    """
    config = load_config(config_path, check_if_valid=False, build_indexes=False)
...
def get_centroid_from_3d(data3d, keys, bounds):
    
    """Determine centroid for each frame manually from 3D keypoint data.
    
    Parameters
    ----------
    data: ndarray, shape (num_segs, seg_length, num_keypoints, 3)
    ...
    -------
    centroid_dict: dict
        Dictionary mapping names to centroids
    
    """
    data = data3d.mean(-2).at[..., 2:].set(0)
    # now data has shape (num_segs, seg_length, 3)
    ...
    for key in set(list(keys)):
        ...
        for (s, e), d in zip(bounds[keys == key], data[keys == key]):
            seq[s:e] = d[: e - s]
        centroid_dict[key] = seq
    return centroid_dict
    
def get_heading_from_3d(data3d, anterior_idxs,
    posterior_idxs, keys, bounds):
    ...
    """
    posterior_loc = data3d[..., posterior_idxs, :2].mean(-2)
    anterior_loc = data3d[..., anterior_idxs, :2].mean(-2)
    data = vector_to_angle(anterior_loc - posterior_loc)
    # now data has shape (num_segs, seg_length)
...
def extract_results(
    ...
    **kwargs
    ...
    if data == None:
        print("No data given, cannot check if 3D or 6D")
    ...
    if data['Y'].shape[-1] > 3: # check if it's 6d, if 3d then centroid and heading already calculated
        centroid = get_centroid_from_3d(coordinates['Y'], *metadata) 
        # shape should be (# sub, # frame + padding, d) for each subject
        heading = get_heading_from_3d(coordinates['Y'], anterior_idxs, posterior_idxs, *metadata) 
        # shape should be (# sub, # frame + padding) for each subject
