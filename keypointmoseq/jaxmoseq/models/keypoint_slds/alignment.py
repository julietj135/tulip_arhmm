...
def angle_to_rotation_matrix(h, d=3):
    ...
    If h = 0, rotation matrix is just the identity
...
def preprocess_for_pca(
    ...
    **kwargs : dict
    ...
    if conf is not None:
        ...
        # outliers = np.zeros_like(conf, dtype=bool)
        if verbose:
            n = outliers.sum()
    ...
    # juliet already aligned and centered in preprocessing
    if Y.shape[-1] > 3:
        v = jnp.zeros(jnp.sum(Y,axis=2).shape)
...
def align_egocentric(Y, anterior_idxs, posterior_idxs, **kwargs):
    ...
    **kwargs : dictp
    ...
    """
    if len(posterior_idxs)==1:
        print("2 indices given for heading")
        posterior_loc = Y[..., posterior_idxs, :2].mean(-2) 
        anterior_loc = Y[..., anterior_idxs, :2].mean(-2) 
        h = vector_to_angle(anterior_loc - posterior_loc)
    else:
        print("3 indicies given for heading (posterior)")
        posterior_loc = Y[..., [posterior_idxs[0]], :].mean(-2)
        mid_loc = Y[..., [posterior_idxs[1]], :].mean(-2) 
        anterior_loc = Y[..., anterior_idxs, :].mean(-2) 
        forward_direction = jnp.cross(mid_loc-posterior_loc, anterior_loc-posterior_loc)
        h = vector_to_angle(forward_direction[...,:2])
