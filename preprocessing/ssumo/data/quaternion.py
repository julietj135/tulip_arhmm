...
def cont6d_to_matrix(cont6d, eps=0):
...
def cont6d_to_matrix_np(cont6d, eps=0):
    assert cont6d.shape[-1] == 6, "The last dimension must be 6"
    x_raw = cont6d[..., 0:3]
    y_raw = cont6d[..., 3:6]
    
    x = x_raw / (np.linalg.norm(x_raw, axis=-1, keepdims=True) + eps)
    z = np.cross(x, y_raw, axisa=-1, axisb=-1)
    z = z / (np.linalg.norm(z, axis=-1, keepdims=True) + eps)

    y = np.cross(z, x, axisa=-1, axisb=-1)
...
def qbetween_np(v0, v1):
    ...
    v = np.cross(v0, v1)
    w = np.sqrt(
        (v0**2).sum(axis=-1, keepdims=True) * (v1**2).sum(axis=-1, keepdims=True)
    ) + (v0 * v1).sum(axis=-1, keepdims=True)
